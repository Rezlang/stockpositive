...
const StockChart: React.FC = () => {
  ...
  const areaData = CANDLEDATA.map(([time, _open, _high, _low, close]) => [
    time,
    close,
  ]);

  // Calculate min/max for y-axis to keep it consistent
  const prices = CANDLEDATA.flatMap(([, open, high, low, close]) => [
    open,
    high,
    low,
    close,
    ...
    <div style={{ padding: "20px" }}>
      <div style={{ marginBottom: "20px" }}>
        <button
          ...
          style={getButtonStyle("candlestick")}
        >
          Candlestick
        </button>
        <button
          onClick={() => setChartType("area")}
          style={{ ...getButtonStyle("area"), marginLeft: "10px" }}
        >
          Area
        </button>
      </div>
